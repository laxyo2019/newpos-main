...
function get_sale_data_row($sale)
{
	...
	// );
	if($CI->Item->check_auth(array('superadmin')))
	{
...
function get_customer_manage_table_headers()
{
	...
	$headers = array(
		...
		array('created_at' => 'Created At'),
		array('last_purchase' => 'Last Purchase'),
	);
...
function get_customer_data_row($person, $stats)
{
	...
	return array (
		...
		'last_purchase' => $person->last_purchase,
...
function get_items_manage_table_headers()
{
	...
	$headers = array(
		...
		array('subcategory' => $CI->lang->line('items_subcategory')),
		array('brand' => $CI->lang->line('items_brand')),
		array('custom2' => 'Size'),
		array('custom3' => 'Color'),
		array('custom5' => 'Expiry Date'),
		array('custom6' => 'Stock Edition'),
		// array('company_name' => $CI->lang->line('suppliers_company_name')),
...
function get_item_data_row($item)
{
	...
	$items_array = array (
		...
		if($CI->Item->check_auth(array('superadmin', 'admin')))
		{
