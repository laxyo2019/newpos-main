...
class Reports extends Secure_Controller
{
	...
	public function get_item_age($item_id, $location_id)
	{
		$trans_date = $this->db->order_by('trans_date', 'desc')
		->where('trans_items', $item_id)
		->where('trans_location', $location_id)
		->limit(1)
		->get('inventory')
		->row('trans_date');

		return $this->Sale->get_days_ago($trans_date);
	}

	public function inventory_age_input()
	{
		$this->load->model('reports/Inventory_summary');

		$data = array();
		$data['days_filter'] = array(
			0 => 'All',
			30 => '30 days',
			90 => '90 days',
			180 => '180 days',
		...
		$this->load->view('reports/inventory_age_input', $data);
	}

	public function inventory_age($location_id = 'all', $days_filter = 'all')
	{
		$inputs = array('location_id' => $location_id, 'days_filter' => $days_filter);

		$this->load->model('reports/Inventory_age');
		$model = $this->Inventory_age;
		...
		foreach($report_data as $row)
		{
			if($row['quantity'] > 0)
			{
				$item_age = $this->get_item_age($row['item_id'], $location_id);
				if($item_age >= $days_filter)
				{
					$tabular_data[] = $this->xss_clean(array(
						'item_id' => $row['item_id'],
						'item_name' => $row['name'],
