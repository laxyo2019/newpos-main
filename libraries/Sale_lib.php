...
class Sale_lib
{
	...
	public function set_return_sale_id($sale_id)
	{
	...
	public function remove_return_sale_id()
	{
		$this->CI->session->unset_userdata('return_sale_id');
	}

	public function apply_credit_note($credit_note_number)
	{
	...
	public function copy_entire_sale($sale_id) //getting any previous sale info from db
	{
		$this->empty_cart();
		$this->remove_customer();
	...
	public function get_discounted_unit($price, $discount)
	{
		$fraction = bcdiv($discount, 100);
		$disc = bcmul($price, $fraction);
		return bcsub($price, $disc);
	}
	...
	{
		// gets the discount amount - eg - 1000$ item with 50% discount returns 500$
		$total = bcmul($quantity, $price);
	...
	public function get_item_tax($quantity, $price, $discount_percentage, $tax_percentage)
	{
		...
		$a = $price * ($tax_percentage * 2);
		$b = 100 + ($tax_percentage * 2);
		return (($a / $b) / 2);
