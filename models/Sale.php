...
class Sale extends CI_Model
{
	...
	public function search($search, $filters, $rows = 0, $limit_from = 0, $sort = 'sales.sale_time', $order = 'desc', $count_only = FALSE)
	{
		...
		{ // show invoices for particular shop only with exception of admin and superadmin
	...
	public function get_days_ago($datetime)
	{
		$db_date = date_create($datetime);
		$today = date_create();
		$diff = date_diff($db_date, $today);
		return ($diff->y * 365) + ($diff->m * 30) + $diff->d;
	}

	public function is_returnable(&$receipt_sale_id)
	{
		if(!empty($receipt_sale_id))
		{
			$pieces = explode(' ', $receipt_sale_id);
			...
			if($this->db->count_all_results('sales_returns') == 0) // if not a credit note
			{
				...
				if($this->get_days_ago($sale_info->row()->sale_time) > 15) // 15 days limit on returns
				{
					return 'EXPIRED';
				}
				else if($this->session->userdata('person_id') !== $sale_info->row()->employee_id)
				{
	...
	public function save($sale_id, &$sale_status, &$items, $customer_id, $employee_id, $comment, $invoice_number, $cashier_id = -1, $work_order_number, $quote_number, $sale_type, $payments, $dinner_table, &$sales_taxes)
	{
		...
		if(!empty($this->session->userdata('billtype')))
		{
			...
			$sales_data['bill_type'] = $this->session->userdata('billtype');
		}
	...
	public function invoice_factory()
	{
