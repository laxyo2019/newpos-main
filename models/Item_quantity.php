...
class Item_quantity extends CI_Model
{
    ...
    public function excel_update($location_detail, $item_id, $location_id)
    {
        $this->db->where('item_id', $item_id);
        $this->db->where('location_id', $location_id);
    ...
    public function get_multilocation_quantity($item_id)
    {
			$multiqty = array();
			$this->db->where('item_id', $item_id);
			...
			{
				$multiqty[$row['location_id']] = $row['quantity']; 
			}
			return $multiqty;
    }
