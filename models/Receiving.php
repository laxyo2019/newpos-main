...
class Receiving extends CI_Model
{
	...
	public function is_my_challan()
	{
		$db_employee_id = $this->db->order_by('receiving_id',"desc")
		->limit(1)
		->get('receivings')
		->row('employee_id');
		$logged_in_user = $this->session->userdata('person_id');
		if($db_employee_id === $logged_in_user)
		{
			return TRUE;
		}
	...
	public function save($owner_id, $items, $supplier_id, $employee_id, $comment, $reference, $payment_type, $receiving_id = FALSE)
	{
		...
		$receivings_data = array(
			...
			'comment' => $comment,
			'reference' => $reference,
			'destination' => $owner_id //stock receiving location owner
		);

		//Run these queries as a transaction, we want to make sure we do all or nothing
		$this->db->trans_start();

		$this->db->insert('receivings', $receivings_data);  // #ENTRY 1
		$receiving_id = $this->db->insert_id();

		// Entry in stock_transfers table
		// $stock_tranfer_data = array(
		// 	'receiving_id' => $receiving_id,
		// 	'source' => $employee_id,
		// 	'destination' => $owner_id
		// );
		// $this->db->insert('stock_transfers', $stock_tranfer_data);
		...
		foreach($items as $line=>$item)
		{
			...
			if($item['quantity'] > 0){
				$stock_movement_data = array( // Entry in inventory_status table
					'receiving_id' => $receiving_id,
					'item_id' => $item['item_id'],
					'quantity' => $item['quantity']
				);

				$this->db->insert('stock_movement', $stock_movement_data);
			}
			...
			if($items_received < 0){ // only subtract stock from source and do not add on destination until accepted
				$item_quantity = $this->Item_quantity->get_item_quantity($item['item_id'], $item['item_location']);

				$this->Item_quantity->save(array('quantity' => $item_quantity->quantity + $items_received, 'item_id' => $item['item_id'], 'location_id' => $item['item_location']), $item['item_id'], $item['item_location']);
			}
			...
			$this->Inventory->insert($inv_data); // New inserts in ospos_inventory
