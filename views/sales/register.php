	...
	<?php echo form_open($controller_name."/change_mode", array('id'=>'mode_form', 'class'=>'form-horizontal panel panel-default sPanel1')); ?>
		...
		<div class="panel-body form-group sPanel1">
			<ul>
				...
				<!-- TAX TYPE -->
				<li class="pull-left"> 
					<?php echo form_dropdown('taxtype', $custom_taxes, $custom_selected_tax, array('class'=>'selectpicker show-menu-arrow', 'data-style'=>'btn-default btn-sm', 'data-width'=>'fit', 'id'=>'taxType')); ?>
				</li>
		...
		<div class="panel-body form-group">
			<ul>
			<?php if(empty($this->session->userdata('sales_cart'))) { ?>
	...
	<table class="sales_table_100" id="register">
		...
		<tbody id="cart_contents">								
			...
			else
			{
				...
				{
			?>
					<?php echo form_open($controller_name."/edit_item/$line", array('class'=>'form-horizontal update2', 'id'=>'cart_'.$line)); ?>
						<tr>
							<td><?php echo anchor($controller_name."/delete_item/$line", '<span class="glyphicon glyphicon-trash"></span>');?></td>
							<td><?php echo $item['item_number']; ?></td>
							<input type="hidden" class="itemId" id="<?php echo $item['item_id']; ?>">
			...
			?>
				<table class="sales_table_100">
					<tr>
						...
						<th style="width: 45%; text-align: right;"><?php echo anchor('customers/view/'.$customer_id, $customer, array('class' => 'modal-dlg', 'style' => 'text-decoration:none; color:#fff', 'data-btn-submit' => $this->lang->line('common_submit'), 'title' => $this->lang->line('customers_update'))); ?></th>
					</tr>
					<?php
					...
					?>
						<tr>
							...
							<th style="width: 45%; text-align: right;">
								<span id="add_credit_note_payment" class="animated flash infinite" style="color:#fff; cursor:pointer"><?php echo $customer_credit_note; ?></span>
							</th>
						</tr>
					<?php
					}
				...
				<?php 
				if(empty($this->session->userdata('sales_payments')))
				{
					echo anchor($controller_name."/remove_customer", '<span class="glyphicon glyphicon-remove">&nbsp</span>' . $this->lang->line('common_remove').' '.$this->lang->line('customers_customer'),
						...
						<div class="form-group form-group-sm">
							...
							<div class="col-xs-6">
								<div class="input-group input-group-sm">
									...
									<?php echo form_input(array('name'=>'sales_invoice_number', 'id'=>'sales_invoice_number', 'class'=>'form-control input-sm', 'value'=> $new_invoice_number ));?>
								</div>
							</div>
						</div>
					</div>
...
      });
  ...
  $('#billType').on('change', function(){
	...
	$('#add_credit_note_payment').on('click', function(){
		$(this).hide();
	  $.post('<?php echo site_url($controller_name."/add_credit_note_payment");?>', {}, function(data) {
			window.location.href = "sales";
  	});
	});
